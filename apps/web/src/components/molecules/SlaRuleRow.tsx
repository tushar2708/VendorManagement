...
export function SlaRuleRow({ stage, label, slaDays: initialDays, escalateAfterBreach: initialEscalate, onSave }: SlaRuleRowProps): React.ReactElement {
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <input type="number" min={1} max={90} value={days} onChange={(e) => setDays(Number(e.target.value))}
            className="w-16 rounded-md border border-slate-300 px-2 py-1.5 text-sm text-center outline-none focus:border-indigo-500" />
          <span className="text-sm text-slate-500">days</span>
        </div>
      </td>
      <td className="px-4 py-3"><Toggle checked={escalate} onChange={setEscalate} /></td>
